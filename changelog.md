...
Implements a dedicated forecast manager job (`fcst_mgr`) that copies GFS
...
**New files:**
- `ush/forecast_mgr.sh` — `fcst_mgr_wait_and_copy` function that polls
...
- `dev/job_cards/rocoto/fcst_mgr.sh` — Rocoto job card for `fcst_mgr`.
...
- `dev/parm/config/gfs/config.resources`: Added `fcst_mgr` resource block
  (2 tasks, 12 h walltime).
- `dev/workflow/rocoto/tasks.py`: Added `'fcst_mgr'` to `VALID_TASKS`.
- `dev/workflow/rocoto/gfs_tasks.py`: Added `fcst_mgr()` method. Depends
  on `atm_products.txt` data file in `COMOUT_CONF` (age ≥ 60 s).
- `dev/workflow/applications/gfs_forecast_only.py`: Added `fcst_mgr` task.
- `dev/workflow/applications/gfs_cycled.py`: Added `fcst_mgr` for GFS run.
