   ...
   if [ "$debug" = ".false." ]; then
      ...
      while [[ $(grep -c '+ rc=0' ${hwrf_nmm_d3_updat_exp2}.out) -ne 1 ]]; do
         ...
         if [ -s return_code_hwrf_nmm_d3.out ]; then
            if [[ $(stat -c %s return_code_hwrf_nmm_d3.out) -ne '0' ]]; then
               if [[ $(awk '{ print $2 }' return_code_hwrf_nmm_d3.out) -ne 'rc=0' ]]; then
                  echo $hwrf_nmm_d3_updat_exp2" job has failed with return code of "$(awk '{ print $2 }' return_code_hwrf_nmm_d3.out)"."
                  rm -f return_code_hwrf_nmm_d3.out
                  exit
               fi
            fi
      ...
      while [[ $(grep -c '+ rc=0' ${hwrf_nmm_d3_contrl_exp1}.out) -ne 1 ]]; do
         ...
         if [ -s return_code_hwrf_nmm_d3.out ]; then
            if [[ $(stat -c %s return_code_hwrf_nmm_d3.out) -ne '0' ]]; then
               if [[ $(awk '{ print $2 }' return_code_hwrf_nmm_d3.out) -ne 'rc=0' ]]; then
                  echo $hwrf_nmm_d3_contrl_exp1" job has failed with return code of "$(awk '{ print $2 }' return_code_hwrf_nmm_d3.out)"."
                  rm -f return_code_hwrf_nmm_d3.out
                  exit
               fi
            fi
      ...
      while [[ $(grep -c '+ rc=0' ${hwrf_nmm_d3_contrl_exp2}.out) -ne 1 ]]; do
         ...
         if [ -s return_code_hwrf_nmm_d3.out ]; then
            if [[ $(stat -c %s return_code_hwrf_nmm_d3.out) -ne '0' ]]; then
               if [[ $(awk '{ print $2 }' return_code_hwrf_nmm_d3.out) -ne 'rc=0' ]]; then
                  echo $hwrf_nmm_d3_contrl_exp2" job has failed with return code of "$(awk '{ print $2 }' return_code_hwrf_nmm_d3.out)"."
                  rm -f return_code_hwrf_nmm_d3.out
                  exit
               fi
            fi
      ...
      /bin/sh $scripts/regression_test.sh $hwrf_nmm_d3_updat_exp1 $hwrf_nmm_d3_updat_exp2 $hwrf_nmm_d3_contrl_exp1 $hwrf_nmm_d3_contrl_exp2 tmpreg_hwrf_nmm_d3 $hwrf_nmm_d3_regression 5 10 2
      ...
      /bin/sh sub_zeus -j $hwrf_nmm_d3_updat_exp1 -p 6/6/ -t 0:35:00 $scripts/hwrf_nmm_d3.sh
