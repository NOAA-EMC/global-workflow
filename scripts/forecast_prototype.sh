...
data_link(){
# data intake for all active components
# Arguments: None 
# 
...
if [ $nfiles -le 0 && $machine != 'sandbox' ]; then
...
case "$1" in
	'ATM')
		echo "SUB: starting to write nems.configure"
		rm -f nems.configure
		cat >nems.configure <<-EOF ###!!! original script likely have bugs here
...
# Argument:
# 
echo "SUB: starting executable launcher."
if [ "$1" = 'sandbox' ]; then
	echo "SUB: (mpirun command here)"
	return
fi
case "$2" in
  	'ATM')
...
data_out()
{
# data output for all active components
# Arguments: None 
# 
...
if [ $SEND = "YES" ]; then
  ...
  if [ $restart_interval -eq 0 -o $restart_interval -eq $FHMAX ]; then
    ...
    RDATE=$($NDATE +$FHMAX $CDATE) ###!!! does this really work?
...
echo "SUB: Output data for FV3 copied"
...
GDATE=$($NDATE -$assim_freq $CDATE) ###!!! Does this really work?
...
echo "MAIN: Finish setting variables\n"

echo "MAIN: Linking input data\n"
data_link
echo "MAIN: Input data linked\n"

echo "MAIN: Writing Namelists\n"
namelist_and_diagtable
...
echo "MAIN: Launching executable"
execution $1 $2

echo "MAIN: Copying Output"
data_out
echo "MAIN: Output copied\n"
