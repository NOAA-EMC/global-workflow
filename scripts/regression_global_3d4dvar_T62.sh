...
#@ step_name=gsi_global_3d4dvar_cntrl
#@ network.MPI=sn_all,shared,us
#@ node = 1
#@ node_usage=not_shared
#@ tasks_per_node=32
...
#@ step_name=gsi_global_3d4dvar_cntrl2
#@ network.MPI=sn_all,shared,us
#@ node = 1
#@ node_usage=not_shared
#@ tasks_per_node=32
...
#@ startdate = 09/27/06 05:00
#@ notification=error
#@ dependency=(gsi_global_3d4dvar_cntrl==0)
#@ restart=no
#@ queue
...
#@ wall_clock_limit = 0:10:00
#@ notification=error
#@ dependency=(gsi_global_3d4dvar_cntrl2==0)
#@ restart=no
#@ queue
...
if [[ "$rc" != "0" ]]; then
   cd $regression_vfydir
   {
    echo ''$exp1_global_3d4dvar_cntrl' has FAILED to run 3dvar to completion, with an error code of '$rc''
   } >> $global_3d4dvar_regression
   $step_name==$rc
   exit
fi

mkdir $noscrub/tmp${global_3d4dvar}
mkdir $control_global_3d4dvar_T62
cp -rp stdout $control_global_3d4dvar_T62
cp -rp fort.220 $control_global_3d4dvar_T62
cp -rp siganl $control_global_3d4dvar_T62
...
if [[ "$rc" != "0" ]]; then
   cd $regression_vfydir
   {
    echo ''$exp2_global_3d4dvar_cntrl' has FAILED to run observer to completion, with an error code of '$rc''
   } >> $global_3d4dvar_regression
   $step_name==$rc
   exit
fi
...
if [[ "$rc" != "0" ]]; then
   cd $regression_vfydir
   {
    echo ''$exp2_global_3d4dvar_cntrl' has FAILED to run id4dvar to completion, with an error code of '$rc''
   } >> $global_3d4dvar_regression
   $step_name==$rc
   exit
fi

mkdir $control_global_3d4dvar_T622
cp -rp stdout $control_global_3d4dvar_T622
cp -rp fort.220 $control_global_3d4dvar_T622
cp -rp siganl $control_global_3d4dvar_T622
