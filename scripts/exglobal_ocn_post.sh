...
if [[ "${RUN}" == sfs ]]; then
    ...
    if [[ -f ${last_fh_output} ]]; then
       ...
       for f in ${file_list_mon}; do
         ...
         cdo monavg "${DATAoutput}/MOM6_OUTPUT/ocn_${f_name:4:4}_${f_name:9:2}_merge.nc" "${COMOUT_OCEAN_NETCDF}/native/${RUN}.t${current_cycle}.monthly_avg.${f_name:4:4}-${f_name:9:2}.nc"

         in_file="${COMOUT_OCEAN_NETCDF}/native/${RUN}.t${current_cycle}.monthly_avg.${f_name:4:4}-${f_name:9:2}.nc"
