...
source="${BASE_CPLIC}/${CPL_ATMIC}/${YMD}${HH}/${MEMDIR}/atmos/gfs_ctrl.nc"
...
for ftype in gfs_data sfc_data; do
  for tt in $(seq 1 6); do
    source="${BASE_CPLIC}/${CPL_ATMIC}/${YMD}${HH}/${MEMDIR}/atmos/${ftype}.tile${tt}.nc"
...
if [[ "${DO_OCN:-}" = "YES" ]]; then
  ...
  source="${BASE_CPLIC}/${CPL_OCNIC}/${PDY}${cyc}/${MEMDIR}/ocean/${OCNRES}/MOM.res.nc"
  ...
  case "${OCNRES}" in
    ...
    "025")  # Only 1/4 degree ocean has MOM.res_[1-4].nc files
      for nn in $(seq 1 4); do
        source="${BASE_CPLIC}/${CPL_OCNIC}/${PDY}${cyc}/${MEMDIR}/ocean/${OCNRES}/MOM.res_${nn}.nc"
        if [[ -f "${source}" ]]; then
...
#  source="${BASE_CPLIC}/${CPL_ICEIC}/${PDY}${cyc}/ice/${ICERES}/cice5_model_${ICERESdec}.res_${PDY}${cyc}.nc"
    source="${BASE_CPLIC}/${CPL_ATMIC}/${YMD}${HH}/${MEMDIR}/ice/${PDY}.${cyc}0000.cice_model.res.nc"
...
if [[ "${DO_WAVE:-}" = "YES" ]]; then
  ...
  for grdID in ${waveGRD}; do  # TODO: check if this is a bash array; if so adjust
    source="${BASE_CPLIC}/${CPL_WAVIC}/${PDY}${cyc}/${MEMDIR}/wave/${grdID}/${PDY}.${cyc}0000.restart.${grdID}"
