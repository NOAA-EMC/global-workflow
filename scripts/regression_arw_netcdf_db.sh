...
#@ error=arw_netcdf_debug_test.e$(jobid)
#@ job_type=parallel
...
aercoef=$crtm_coef/AerosolCoeff/AerosolCoeff.bin
cldcoef=$crtm_coef/CloudCoeff/CloudCoeff.bin
...
if [[ "$rc" = "0" ]]; then
   ...
   {
    echo
    echo 'arw_netcdf debug test has passed'
