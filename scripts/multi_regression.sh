...
if [ -d /da ]; then
#For WCOSS
   echo "/da/save/$LOGNAME/EXP-meta_data-read_files/scripts/regression_var.sh" > regression_var.out
elif [ -d /scratch1/portfolios/NCEPDEV/da ]; then
#For Zeus
   echo "/scratch1/portfolios/NCEPDEV/da/save/$LOGNAME/trunk/scripts/regression_var.sh" > regression_var.out
fi
