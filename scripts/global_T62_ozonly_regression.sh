...
job[1]=$global_T62_ozonly_updat_exp1
job[2]=$global_T62_ozonly_updat_exp2
job[3]=$global_T62_ozonly_contrl_exp1
job[4]=$global_T62_ozonly_contrl_exp2
...
if [ "$debug" = ".false." ]; then
   ...
   /bin/sh $scripts/regression_test.sh ${job[1]} ${job[2]} ${job[3]} ${job[4]} tmp62_ozonly $global_ozonly_regression 10 8 4

else
  
   if [ "$machine" = "Theia" ]; then
      /bin/sh $sub_cmd -j ${job[1]} -p ${popts[1]} -t 0:45:00 $scripts/global_T62_ozonly.sh
   elif [ "$machine" = "WCOSS" ]; then
      /bin/sh $sub_cmd -j ${job[1]} -p ${popts[1]} -t 0:45:00 $scripts/global_T62_ozonly.sh
   fi
