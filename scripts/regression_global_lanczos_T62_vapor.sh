...
#@ step_name=gsi_global_lanczos_update
...
#@ step_name=gsi_global_lanczos_update2
#@ network.MPI=sn_all,shared,us
#@ node = 3
#@ node_usage=not_shared
#@ tasks_per_node=32
...
#@ notification=error
#@ restart=no
#@ dependency=(gsi_global_lanczos_update==0)
#@ queue

#@ step_name=gsi_global_lanczos_benchmark
#@ network.MPI=sn_all,shared,us
#@ node = 2
#@ node_usage=not_shared
#@ tasks_per_node=32
...
#@ notification=error
#@ restart=no
#@ dependency=(gsi_global_lanczos_update2==0)
#@ queue

#@ step_name=gsi_global_lanczos_benchmark2
#@ network.MPI=sn_all,shared,us
#@ node = 3
#@ node_usage=not_shared
#@ tasks_per_node=32
...
#@ notification=error
#@ restart=no
#@ dependency=(gsi_global_lanczos_benchmark==0)
#@ queue
...
#@ notification=error
#@ restart=no
#@ dependency=(gsi_global_lanczos_benchmark2==0)
#@ queue

. ./regression_var.sh

case $LOADL_STEP_NAME in
  gsi_global_lanczos_update)
...
if [[ "$rc" != "0" ]]; then
   ...
   {
    echo ''$exp1_global_lanczos_sub_1node' has failed to run to completion, with an error code of '$rc''
...
if [[ "$rc" != "0" ]]; then
   cd $regression_vfydir
   {
    echo ''$exp2_global_lanczos_sub_2node' has failed to run to completion, with an error code of '$rc''
   } >> $global_lanczos_regression
   $step_name==$rc
   exit
fi
...
if [[ "$rc" != "0" ]]; then
   cd $regression_vfydir
   {
    echo ''$exp1_global_lanczos_bench_1node' has failed to run to completion, with an error code of '$rc''
   } >> $global_lanczos_regression
   $step_name==$rc
   exit
fi
...
if [[ "$rc" != "0" ]]; then
   cd $regression_vfydir
   {
    echo ''$exp2_global_lanczos_bench_2node' has failed to run to completion, with an error code of '$rc''
   } >> $global_lanczos_regression
   $step_name==$rc
   exit
fi
...
exp1=$exp1_global_lanczos_sub_1node
exp2=$exp1_global_lanczos_bench_1node
exp3=$exp2_global_lanczos_sub_2node
...
exp1_scale=$exp2_global_lanczos_sub_2node
exp2_scale=$exp2_global_lanczos_bench_2node
...
cd $scripts
rm -f regression_test.gsi_global_lanczos_update.e*
rm -f regression_test.gsi_global_lanczos_update2.e*
rm -f regression_test.gsi_global_lanczos_benchmark.e*
rm -f regression_test.gsi_global_lanczos_benchmark2.e*
