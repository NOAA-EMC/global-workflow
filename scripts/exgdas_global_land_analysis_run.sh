...
#                      .                                             .
# Script name:         exgdas_global_land_analysis_run.sh
# Script description:  Runs the global land analysis with FV3-JEDI
#
# Author: Jiarui Dong        Org: NCEP/EMC     Date: 2023-01-15
#
...
################################################################################
# run executable
export OMP_NUM_THREADS=$NTHREADS_LANDANAL
export pgm=$JEDIVAREXE
. prep_step
$APRUN_LANDANAL $DATA/fv3jedi_var.x $DATA/fv3jedi_var.yaml 1>&1 2>&2
export err=$?; err_chk
