...
cat << EOF > gsiparm.anl
 ...
 &SINGLEOB_TEST
   ...
   oblat=45.,oblon=270.,obpres=850.,obdattim=${arw_binary_adate},
   obhourset=0.,
 /
EOF
...
if [[ "$exp" = $arw_binary_updat_exp1 ]]; then
   $ncp $gsiexec_updat ./gsi.x
elif [[ "$exp" = $arw_binary_updat_exp2 ]]; then
   $ncp $gsiexec_updat ./gsi.x
elif [[ "$exp" = $arw_binary_contrl_exp1 ]]; then
   $ncp $gsiexec_contrl ./gsi.x
elif [[ "$exp" = $arw_binary_contrl_exp2 ]]; then
   $ncp $gsiexec_contrl ./gsi.x
fi
...
$ncp $arw_binary_obs/${prefixo}.satwnd.$suffix  ./satwnd
...
if [[ "$io_format" = "binary" ]]; then
   $ncp $arw_binary_ges/wrfinput_d01_2010-07-24_12:00:00       ./wrf_inout_orig
elif [[ "$io_format" = "netcdf" ]]; then
