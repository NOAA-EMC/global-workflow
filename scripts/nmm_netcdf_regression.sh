...
if [ "$machine" = "Zeus" -o "$machine" = "Theia" ]; then
   ...
   if [ "$debug" = ".false." ]; then
      /bin/sh sub_zeus -j $nmm_netcdf_updat_exp1 -t 0:30:00 -p 4/2/0 -r /1 $scripts/nmm_netcdf.sh
      ...
      /bin/sh sub_zeus -j $nmm_netcdf_updat_exp2 -t 0:25:00 -p 4/4/0 -r /1 $scripts/nmm_netcdf.sh
      ...
      /bin/sh sub_zeus -j $nmm_netcdf_contrl_exp1 -t 0:30:00 -p 4/2/0 -r /1 $scripts/nmm_netcdf.sh
      ...
      /bin/sh sub_zeus -j $nmm_netcdf_contrl_exp2 -t 0:25:00 -p 4/4/0 -r /1 $scripts/nmm_netcdf.sh
