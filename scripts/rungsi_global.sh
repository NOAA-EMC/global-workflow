...
datobs=$basedir/wx20rt/gsi_anl/cases/global/sigmap/$adate
datges=$datobs
...
tmpdir=/ptmp/$LOGNAME/tmp${JCAP}/${exp}
...
if [[ "$JCAP" = "382" ]]; then
   ...
   echo "Test case $adate not available for JCAP = $JCAP"
   exit
fi
export NLAT=$((${LATA}+2))
...
cat << EOF > gsiparm.anl
 &SETUP
   ...
   ndat=59,jpch=843,npred=5,iguess=-1,
 ...
 /
 &STRONGOPTS
   jcstrong=.true.,nstrong=1,nvmodes_keep=8,period_max=6.,period_width=1.5,nstrong_end=1,update_end=.false.,
 /
 &OBSQC
 ...
 &OBS_INPUT
   ...
   dfile(15)='sbuvbufr',  dtype(15)='sbuv2',     dplat(15)='n18',       dsis(15)='sbuv2_n18',           dval(15)=1.0,  dthin(15)=0,
   ...
   dfile(28)='amsuabufr', dtype(28)='amsua',     dplat(28)='n16',       dsis(28)='amsua_n16',           dval(28)=1.0,  dthin(28)=2,
...
berror=$fixpath/global_berror.l${LEVS}y${NLAT}.f77
specoef=$fixpath/global_CRTM_SpcCoeff.f77
trncoef=$fixpath/global_CRTM_TauCoeff.f77
emiscoef=$fixpath/global_CRTM_EmisCoeff.f77
aercoef=$fixpath/global_CRTM_AerosolCoeff.f77
cldcoef=$fixpath/global_CRTM_CldCoeff.f77
satinfo=$fixpath/global_satinfo.txt
satangl=$fixpath/global_satangbias.txt
pcpinfo=$fixpath/global_pcpinfo.txt
ozinfo=$fixpath/global_ozinfo.txt
errtable=/nwprod/fix/prepobs_errtable.global
convinfo=$fixpath/global_convinfo.txt
...
$ncp $datobs/${prefix_obs}.osbuv.${suffix}    ./sbuvbufr
$ncp $datobs/${prefix_obs}.omto3.tm00.txt     ./omi
$ncp $datobs/${prefix_obs}.goesnd.${suffix}   ./gsndrbufr
...
list="03 06 09"
for fh in $list; do
   if [[ -s $datges/${prefix_sfc}.bf$fh.linear ]]; then
      $ncp $datges/${prefix_sfc}.bf$fh.linear     ./sfcf$fh
   else
      $ncp $datges/${prefix_sfc}.bf$fh            ./sfcf$fh.quad
   fi
done
...
# NOTE:  If JCAP=62, no need to chgres, just rename files
...
#  Uncomment line below if $adate < 2006082212
#  export LANDICE_OPT=5
