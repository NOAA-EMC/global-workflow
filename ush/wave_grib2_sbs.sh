...
if [[ ${fhr} -gt 0 ]]; then
    ${WGRIB2} gribfile -set_date "${PDY}${cyc}" -set_ftime "${fhr} hour fcst" -set_grib_type c2 -g2clib 0 -grib "${outfile}"
    err=$?
else
    ${WGRIB2} gribfile -set_date "${PDY}${cyc}" -set_ftime "${fhr} hour fcst" \
        -set table_1.4 1 -set table_1.2 1 -set_grib_type c2 -g2clib 0 -grib "${outfile}"
    err=$?
fi
