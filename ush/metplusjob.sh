...
##   grid2grid_typelist, grid2obs_typelist, vhr_rain, ftypelist, ptyplist, anltype, rain_bucket,
...
rain_bucket=${rain_bucket:-"6"}                                             # accumulation bucket in hours: 6 (future support: other accums, bucket=0 -- continuous accumulation)
...
if [ $VRFY_GRID2GRID_STEP1 = YES ] ; then
    ...
    for vhr in $vhrlist; do
        VDATE=${VDATESTART}${vhr}
        while [ $VDATE -le ${VDATEEND}${vhr} ] ; do
            export VDATE=$VDATE
            ...
            while [ $nn -le $nexp ] ; do
                ...
                for type in $grid2grid_typelist ; do
                    if [ $type = pres ]; then
                        ...
                        if [ -s $anlfile ]; then
                            grid2grid_run_typelist+=(pres)
                        else
                            echo "ERROR: ${anlfile} doesn't exist or zero-sized. SKIPPING grid-to-grid pres verification for this date." 
                    ...
                    if [ $type = anom ]; then
                        anlfile=${rundir_g2g1}/data/${exp}/pgbanl${cdump}${VDATE}
                        if [ -s $anlfile ]; then
                            grid2grid_run_typelist+=(anom)
                        else
                            echo "ERROR: ${anlfile} doesn't exist or zero-sized. SKIPPING grid-to-grid anom verification for this date." 
                ...
                export typelist="${grid2grid_run_typelist[@]}"
                ...
                if [ ${#grid2grid_run_typelist[@]} -ge 1 ] ; then
                    ...
                    if [ $MPMD = YES ] ; then
                        ...
                        for type in "${grid2grid_run_typelist[@]}" ; do
                            export type=$type
                            export savedir=${metplussave}/grid2grid/${type}/${vhr}Z/${exp}
                            mkdir -p ${savedir}
                            if [ ${type} = anom ] ; then
        ...
        while [ $nn -le $nexp ] ; do
            ...
            for type in $grid2obs_typelist ; do
                if [ $type = upper_air ]; then
                    ...
                    echo "ERROR: grid-to-obs ${type} is not supported."
                    echo "grid-to-obs ${type} is not supported."  >> ${rundir_g2o1}/make_met_data/${type}/error_${VDATE}.txt
                fi
            done
            ...
            if [ ${#grid2obs_run_typelist[@]} -ge 1 ] ; then
                ...
                if [ $MPMD = YES ] ; then
                    ...
                    for type in "${grid2obs_run_typelist[@]}" ; do
                        export type=${type}
                        ${metplushome}/ush/master_metplus.py -c ${metplusconfig}/metplus_config/METplus-${METPLUSver}/mpmd_confs/grid2obs_${type}_step1c.conf -c ${metplusconfig}/machine_config/machine.${machine}
                        for fcycl in $fcyclist ; do
                            export savedir=${metplussave}/grid2obs/${type}/${fcycl}Z/${exp}
                            mkdir -p ${savedir}
                            cp ${rundir_g2o1}/VSDB_format/${type}/${fcycl}Z/${exp}/*.stat ${savedir}/.
                        done
                    done
                #run in serially if MPMD=NO
                else
...
if [ $VRFY_PRECIP_STEP1 = YES ] ; then
    ...
    export rain_bucket=${rain_bucket}                                                       #accumulation bucket in hours: 6 (future support: other accums, bucket=0 -- continuous accumulation)
    #create directories for output
    ...
    nexp=`echo $expnlist |wc -w`
    VDATE=${VDATESTART_precip}12
    while [ $VDATE -le ${VDATEEND_precip}12 ] ; do
        export VDATE=$VDATE
