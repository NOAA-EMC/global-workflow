...
export homesyndir=${homesyndir:-$PACKAGEROOT/util}
...
do
    ...
    if [[ ! ( "$pcount" -lt 30 ) ]] ; then
      echo "Hit maximum number of postmsg commands for tcvitals usage at hour 0.  Will stop warning about that, to avoid spamming log."
      break
    fi
    echo "$line"
    pcount=$(( pcount + 1 ))
