...
# Check if file existing in checkpointing situation
  ifile="$WAV_MOD_TAG.out_grd.$grdID.${YMD}.${HMS}"

  if [ -s ${COMOUT}/rundata/${ifile} ]; then
    set +x
    echo ' '
...
# 1.c Save in /com
    ...
    then
      set +x
      echo "   Saving GRID file as $COMOUT/rundata/${ifile}"
      [[ "$LOUD" = YES ]] && set -x
