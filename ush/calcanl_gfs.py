...
def calcanl_gfs(DoIAU, l4DEnsVar, Write4Danl, ComOut, APrefix,
    ...
    if DoIAU and l4DEnsVar and Write4Danl:
        for fh in IAUHH:
            if fh == 6:
                ...
                if Run in ["gdas", "gfs"]:
                    ...
                    gsi_utils.copy_file(ExecAnl, CalcAnlDir + '/calc_anl.x')
                    gsi_utils.link_file(RunDir + '/siginc.nc', CalcAnlDir + '/siginc.nc.06')
                    gsi_utils.link_file(ComOut + '/' + APrefix + 'ensres.anl.atm.nc', CalcAnlDir + '/anl.ensres.06')
                ...
                if os.path.isfile('sigi' + format(fh, '02') + '.nc'):
                    ...
                    if not os.path.exists(CalcAnlDir6):
                        gsi_utils.make_dir(CalcAnlDir6)
                    gsi_utils.link_file(ComOut + '/' + APrefix + 'ensres.anl.atm.i' + format(fh, '03') + '.nc',
        ...
        gsi_utils.copy_file(ExecAnl, CalcAnlDir + '/calc_anl.x')
        gsi_utils.link_file(RunDir + '/siginc.nc', CalcAnlDir + '/siginc.nc.06')
        gsi_utils.link_file(ComOut + '/' + APrefix + 'ensres.anl.nc', CalcAnlDir + '/anl.ensres.06')
        gsi_utils.link_file(ComIn_Ges + '/' + GPrefix + 'ensres.atm.f006.nc', CalcAnlDir + '/ges.ensres.06')
