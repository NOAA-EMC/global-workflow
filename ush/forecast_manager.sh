...
while [[ ${remaining} -gt 0 ]]; do
    ...
    # FCST_MANAGER_DONE_IDLE_MAX idle cycles (default 3) exit with a warning for
    # any entries that the model never produced (e.g. optional GOCART output types).
    if [[ -n "${FCST_DONE_SENTINEL:-}" && -f "${FCST_DONE_SENTINEL}" ]]; then
        if [[ ${remaining} -lt ${remaining_before} ]]; then
            fcst_done_idle=0
        else
            ((fcst_done_idle++)) || true
            idle_max=${FCST_MANAGER_DONE_IDLE_MAX:-3}
