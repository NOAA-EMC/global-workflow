...
for ((i = 0; i < nm; i += chunk_size)); do
    ...
    err=$?
    if [[ ${err} -ne 0 ]]; then
        echo "ERROR: MPMD job failed for ${chunk_file} with exit code ${err}"
        exit "${err}"
    fi
    # Call cat_outputs and error if no outputs are found.
    cat_outputs "true"
    _cat_err=$?
    if [[ ${_cat_err} -ne 0 ]]; then
...
if [[ -s mpmd.out ]]; then
    cat mpmd.out
    if grep -E -q "FATAL ERROR|ERROR:" mpmd.out; then
        echo "=========================================================================" >&2
        echo "CRITICAL FAILURE: Detected 'FATAL ERROR' or 'ERROR' in mpmd.out log!" >&2
        echo "=========================================================================" >&2
        err=1
    fi
else
