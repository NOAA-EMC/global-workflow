      program cnvgrib2_wafs
C$$$  MAIN PROGRAM DOCUMENTATION BLOCK
C  
C MAIN PROGRAM:  cnvgrib2_wafs
...
C  GET ARGUMENTS
      ...
      IF(NARG.lt.3) THEN       ! may be a problem with args
         IF(NARG.eq.0) THEN
            !CALL ERRMSG('cnvgrib2_wafs:  Incorrect usage')
            call usage(0)
            CALL ERREXIT(2)
     ...
     &      table_ver .gt. mastertab ) then
            call usage(0)
            call errmsg ('  ')
            call errmsg('cnvgrib2_wafs: cannot change to master '// 
     ...
     &                  curmastertab_ver)
         ...
         if ( (ipack.ne.-1).and.(outver.eq.1) ) then
            CALL ERRMSG('cnvgrib2_wafs: -pxx option ignored when '//
     &                   'using -g21')
         endif
         !
         !   Must have -g option
         ...
         if ( (inver.eq.0).or.(outver.eq.0) ) then
            CALL ERRMSG('cnvgrib2_wafs: must use one -gxx option')
            call usage(0)
            CALL ERREXIT(2)
         endif
         !
     ...
     &                       ipack.ne.32) ) then
            CALL ERRMSG('cnvgrib2_wafs: -m or -m0 option ignored '//
...
!
      ...
      if (IOS.NE.0) then
         call errmsg('cnvgrib2_wafs: cannot open input GRIB file '//
     &               gfilein(1:NCGB))
         call errexit(3)
      endif
      ...
      if (IOS.NE.0) then
         call errmsg('cnvgrib2_wafs: cannot open output GRIB file '//
     &               gfileout(1:NCGB))
         call errexit(4)
      endif
!
...
C
C$$$
         character(len=20) :: cnvgrib2_wafs_ver="cnvgrib2_wafs-1.2.2"
         integer,intent(in) :: iopt 

         if ( iopt.eq.0 ) then
         call errmsg ('  ')
         call errmsg('Usage: cnvgrib2_wafs [-h] {-g12|-g21|-g22} '//
     &        ' [-m|-m0] [-nv] [-mastertable_ver_x]')
         call errmsg('               [{-p0|-p2|-p31|-p32|-p40'//
     &               '|-p41}]  ingribfile   outgribfile')
         call errmsg ('  ')
         call errmsg('Usage: cnvgrib2_wafs  -h  For helps and '//
     &                                  'show all options') 
         call errmsg ('  ')
         endif
         ...
         if ( iopt.eq.1 ) then
            call errmsg ('  ')
            call errmsg('cnvgrib2_wafs:  version '//cnvgrib2_wafs_ver)
            call errmsg ('  ')
