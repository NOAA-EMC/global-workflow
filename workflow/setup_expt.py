...
def input_args(*argv):
    ...
    def _common_args(parser):
        ...
        parser.add_argument('--resdetatm', help='atmosphere resolution of the deterministic model forecast',
        ...
        parser.add_argument('--resdetocean', help='ocean resolution of the deterministic model forecast',
                            type=float, required=False, default=-1.0)  # -1.0 means determine from resdetatm (limited combinations will be available)
    ...
    def _gfs_or_gefs_ensemble_args(parser):
        parser.add_argument('--resensatm', help='atmosphere esolution of the ensemble model forecast',
...
def validate_user_request(host, inputs):
    ...
    for attr in ['resdetatm', 'resensatm']:
        try:
...
def get_ocean_resolution(resdetatm):
    """
    ...
    atm_to_ocean_map = {
        ...
        96: 5.0, 48: 5.0}
    try:
        return atm_to_ocean_map[resdetatm]
    except KeyError:
...
def main(*argv):
    ...
    if user_inputs.resdetocean < 0:
        user_inputs.resdetocean = get_ocean_resolution(user_inputs.resdetatm)
