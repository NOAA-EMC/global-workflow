...
! THE GRID CONTAINS TWO EXTRA ROWS FOR EACH POLE.  IPOLATES
! ASSUMES NO POLE POINTS, SO THESE ARE REMOVED.  ALSO, IPOLATES
! EXPECTS ROW 1 TO BE THE NORTH POLE, SO THE ROWS ARE FLIPPED.
!-----------------------------------------------------------------
 ...
 ERROR=NF90_OPEN(TRIM(GSI_FILE),NF_NOWRITE,NCID)
...
! FLIP J DIRECTION.  REMOVE POLE POINTS.

 DO J = 1, JDIM_GAUS
   SLMSK_GAUS(:,J) = IDUMMY(:,JDIM_GAUS+2-J)
   DTREF_GAUS(:,J) = DUMMY(:,JDIM_GAUS+2-J)
 ENDDO
