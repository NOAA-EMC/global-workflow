...
!-------------------------------------------------------------------------
 ...
 TYPE(ESMF_IndexFlag)                 :: ESMF_INDEX_DELOCAL
 INTEGER                              :: status
 ...
 IF(ASSOCIATED(F90Array)) THEN
      ...
      ESMFArray = ESMF_ArrayCreate(F90Array, distgrid, &
          name = name, indexFlag=ESMF_INDEX_DELOCAL, rc = status)
...
!           ESMF state to extract F90 array from
 TYPE(ESMF_State)                          :: state  
!           name of the ESMF array to extract the fortran array from
 CHARACTER(LEN = *)                        :: name   

 INTEGER,            INTENT(in)            :: localPE ! PE id of the local PE.

!           Name of the nested ESMF state which contains the ESMF array.
 CHARACTER(LEN = *), INTENT(in), OPTIONAL  :: nestedStateName   
