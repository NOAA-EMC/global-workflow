...
The accepted arguments is a list of systems to be built.  This includes builds for **GFS**, **GEFS**, and **SFS** forecast-only experiments, **GSI** and **GDASApp**-based DA for **cycled GFS** experiments.  See :ref:`feature availability <feature_availability>` to see which system(s) are available on each supported system.
