...
mkdir -p $fv3gfs
cd $fv3gfs
svn co $fv3gfs_tag $pslot

cd $fv3gfs/$pslot/global_shared.v15.0.0/sorc
sh checkout.sh
sh build_all.sh $machine

cd $fv3gfs/$pslot/gfs_workflow.v15.0.0/ush
python setup_expt.py --pslot $pslot --comrot $comrot --expdir $expdir --idate $idate --edate $edate --icsdir $icsdir --configdir ../config
