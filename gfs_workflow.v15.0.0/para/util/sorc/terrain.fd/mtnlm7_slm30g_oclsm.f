...
!
      real, dimension(jm)    :: COSCLT, WGTCLT, RCLT, XLAT
      REAL, dimension(im,jm) :: SLM, ORO, ORF, oro_s
      real                   :: ors(nw), diffx(jm/2)
...
!
      ...
      FILTER = .TRUE.        ! Spectral Filter defaults true and set by NF1 & NF0
...
! --- metadata to distinguish its disposition.  The AI below mitigates this.
      ...
      if (mskocn == 1) then
        write(0,*) ' Ocean Model LSM Present and '
        write(0,*) ' Overrides OCEAN POINTS in LSM: mskocn=',mskocn
        if (notocn == 1) then
          write(0,*) ' Ocean LSM Reversed:  NOTOCN=',notocn
        endif
      endif
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   12   FORMAT(80I1)
        if (ios /= 0) then 
          MSKSRC = -1
          write(0,*)' navy10 lake mask rd fail - ios,MSKSRC:',ios,MSKSRC
        endif
      ELSE
...
! --------------
        IF (istat /= 0) then ! --- When UMD read fails attempt to read navy 10'
          print *,' UMD lake mask readd err -- trying navy 10',istat
          MSKSRC = 0
...
! --- open to fort.25 with link to file in script
        ...
        if (istat /= 0) then
          mskocn = 0
          print *,' Ocean lsm file Open failure: mskocn,istat=',
     &              mskocn,istat
        else
          mskocn = 1
          print *,' Ocean lsm file Opened OK: mskocn,istat=',
     &              mskocn,istat
        endif
! --- Read it in
        ios  = 0
...
!     continue using ZSLMX
          print *,' Rd fail: Ocean lsm - continue, mskocn,ios=',
     &              mskocn,ios
        else
          mskocn = 1
...
! --- remember, that lake mask is in zslm to be assigned in MAKEMT.
          if ( mskocn == 1 ) then
            ...
            DO J = 1,JM
              ...
              enddo
            enddo
            print *,' OCLSM',OCLSM(1,1),OCLSM(50,50),OCLSM(75,75),
     &                OCLSM(IM,JM)
            print *,' SLMI:',SLMI(1,1),SLMI(50,50),SLMI(75,75),
     &                SLMI(IM,JM)
! --- Diag
...
!
      ...
      write(0,*)' SLM(itest,jtest)=',slm(itest,jtest),itest,jtest
...
! ---  Land and Lakes and all other atm elv moments remain unchanged.
      if ( mskocn == 1 ) then
        ...
        DO j = 1,jm
          DO i = 1,numi(j)
            if (abs (oro(i,j)) < 1. ) then
              slm(i,j) = slmi(i,j)
            else
              if ( slmi(i,j) == 1. .and. slm(i,j) == 1) slm(i,j) = 1
              if ( slmi(i,j) == 0. .and. slm(i,j) == 0) slm(i,j) = 0
              if ( slmi(i,j) == 0. .and. slm(i,j) == 1) slm(i,j) = 0
              if ( slmi(i,j) == 0. .and. slm(i,j) == 0) slm(i,j) = 0
            endif
          enddo
        enddo
...
! --- SLM
      ...
      if (iret /= 0) print *,' BAOPEN ERROR UNIT 56: IRET=',IRET
      CALL PUTGB(56,IM*JM,KPDS,KGDS,LB,SLM,IRET)
      print *,' SLM: putgb-KPDS(22,5),iret:',KPDS(22),KPDS(5),IRET
      if (iret /= 0) print *,' SLM PUTGB ERROR:  UNIT 56: IRET=',IRET
