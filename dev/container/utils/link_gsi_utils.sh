...
for model in adderrspec adjustps calc_analysis calc_increment_ens \
             calc_increment_ens_ncio calc_increment_ncio calc_increment_serial \
             getnstensmeanp getsfcensmeanp getsfcnstensupdp getsigensmeanp_smooth \
             getsigensstatp gribmean interp_inc \
             recenterens_ncio recenterncio_hybgain recenternemsiop_hybgain recentersigp
do
...
# for Ursa
 ...
 singularity exec \\
    ...
    ${bindings} \\
    ${container} \\
    ${HOMEgfs}/dev/container/env/gsiutils-env.sh \\
    ${HOMEgfs}/sorc/gsi_utils.fd/install/bin/${model}.x \\
    "\$@"
EOF_URSA
    ;;
