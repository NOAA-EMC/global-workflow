...
The aux.xml will be rendered into the directory ``EXP_aux`` specified in the
configuration YAML. The config files from parm/config/gfs (or the override
directory specified by --gfs_config_dir) will be linked into EXP_aux for use by
the workflow.

This script is designed to facilitate the setup of an auxiliary workflow for
management of non-operational tasks and is intended to run alongside a real-time
or near-real-time ecflow workflow.

It reads a configuration YAML file, contents of parm/config/gfs (not
dev/parm/config/gfs) and renders a Jinja2-templated XML workflow file (aux.xml.j2)
into a Rocoto XML workflow file (aux.xml) for use with the Rocoto workflow
manager.
...
Instructions:
    ...
    * If you are regenerating an auxiliary workflow XML, start by backing up
    your current XML and database. Ensure that you save them into a new location,
    as the current location will be overwritten.

    Start by modifying the dev/parm/aux/aux.yaml file
      * start_date
          - YYYYMMDDHH00 format
          - When regenerating the auxiliary in place, ensure that the start_date
            remains the same as the SDATE in the current XML.
      * end_date
          – set to some future date
          - can be short into the future or the same as start_date for testing
      * HOMEglobal
          - only uncomment/set this if the setup needs to point somewhere
            external to the workflow you are running from.
      * EXP_aux
          – The expdir for the case. The rendered XML will land here.
      * ECF_OUT_gfs
          – Where the ecflow scripts are written to, including /today
          - e.g. /lfs/h2/emc/gfstemp/emc.global/ecflow/output/today
      * COM_aux
          - The auxiliary's COM (separate from the experiment's COM)
      * ARCDIR
          - Where to place the archive of model forecasts and METplus .stat files
            on local disk.
      * ATARDIR
          - Where to archive the ROTDIR on HPSS.
      * DATAROOT_aux
          – DATAROOT for the auxiliary system. E.g. /lfs/h2/emc/stmp/emc.global/RUNDIRS/aux_gfs.

Warning:
    This auxiliary workflow cannot currently be used in the develop branch. It's design
    assumes a pre-production environment where parm/config/{MODEL} is fully rendered and
    available for use and a fully functional ecflow workflow is in place under ecf/.
    This can be achieved by first rendering the parm/config/{MODEL} files and copying the
    appropriate dev/ecf/{MODEL} and dev/ecf/common files to the ecf/ directory.

TODOs:
    - Write a utility to render the parm/config/{MODEL} files from the
      dev/parm/config/{MODEL} templates and copy appropriate dev/ecf files.
    - Write a test script to validate the rendered aux.xml against the Rocoto schema.
    - Add more robust error handling for missing or malformed configuration files.
    - Implement unit tests for the date calculation and template rendering functions.
    - Add support for additional command-line arguments to customize behavior.
    - Improve logging to include more detailed information about the rendering process.
    - Provide defaults for the aux.yaml settings to allow for easier initial setup and
      testing.
"""
