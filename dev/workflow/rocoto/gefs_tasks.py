...
class GEFSTasks(Tasks):
    ...
    def gen_control_ic(self):
        ...
        task_dict = {'task_name': task_name,
                     ...
                     'envars': self.envars,
                     'cycledef': self.run,
                     'command': f'{self.HOMEgfs}/dev/job_cards/rocoto/gen_control_ic.sh',
                     'job_name': f'{self.pslot}_{task_name}_@H',
                     'log': f'{self.rotdir}/logs/@Y@m@d@H/{task_name}.log',
                     'maxtries': '&MAXTRIES;'
                     }
    ...
    def awips(self):
        ...
        task_dict = {'task_name': task_name,
                     ...
                     'envars': postenvars,
                     'cycledef': self.run,
                     'command': f'{self.HOMEgfs}/dev/job_cards/rocoto/awips.sh',
                     'job_name': f'{self.pslot}_{task_name}_@H',
                     'log': f'{self.rotdir}/logs/@Y@m@d@H/{task_name}.log',
                     'maxtries': '&MAXTRIES;'}
    ...
    def wavepostgridded(self):
        ...
        task_name = f'{self.run}_wavepostgridded_mem#member#_#fhr_label#'
        task_dict = {'task_name': task_name,
                     'resources': resources,
                     'dependency': dependencies,
        ...
        fhr_metatask_dict = {'task_name': f'{self.run}_wavepostgridded_#member#',
                             'task_dict': task_dict,
                             'var_dict': fhr_var_dict}

        member_var_dict = {'member': ' '.join([f"{mem:03d}" for mem in range(0, self.nmem + 1)])}
        member_metatask_dict = {'task_name': f'{self.run}_wavepostgridded',
                                'task_dict': fhr_metatask_dict,
                                'var_dict': member_var_dict}
    ...
    def wave_stat(self):
        ...
        deps = []
        for member in range(0, self.nmem + 1):
            task = f'{self.run}_wavepostgridded_mem{member:03d}_#fhr_label#'
            dep_dict = {'type': 'task', 'name': task}
            deps.append(rocoto.add_dependency(dep_dict))
        dependencies = rocoto.create_dependency(dep_condition='and', dep=deps)
        ...
        task_dict = {'task_name': task_name,
                     ...
                     'cycledef': self.run,
                     'command': f'{self.HOMEgfs}/dev/job_cards/rocoto/wave_stat.sh',
                     'job_name': f'{self.pslot}_{task_name}_@H',
                     'log': f'{self.rotdir}/logs/@Y@m@d@H/{task_name}.log',
                     'maxtries': '&MAXTRIES;'
                     }
    ...
    def wave_stat_pnt(self):
        ...
        task_dict = {'task_name': task_name,
                     ...
                     'envars': self.envars,
                     'cycledef': self.run,
                     'command': f'{self.HOMEgfs}/dev/job_cards/rocoto/wave_stat_pnt.sh',
                     'job_name': f'{self.pslot}_{task_name}_@H',
                     'log': f'{self.rotdir}/logs/@Y@m@d@H/{task_name}.log',
                     'maxtries': '&MAXTRIES;'
                     }
    ...
    def extractvars(self):
        deps = []
        if self.options['do_wave']:
            dep_dict = {'type': 'metatask', 'name': f'{self.run}_wavepostgridded_#member#'}
            deps.append(rocoto.add_dependency(dep_dict))
