...
# Files needed by ${USHgcafs}/interp_atmos_master.sh
...
for ((nset = 1; nset <= downset; nset++)); do
    ...
    for ((iproc = 1; iproc <= nproc; iproc++)); do
        ...
        echo "${USHgcafs}/interp_atmos_master.sh ${input_file} ${output_file_prefix} ${grid_string}" >> "${DATA}/cmdfile"
    ...
    "${USHgcafs}/run_mpmd.sh" "${DATA}/cmdfile" && true
    export err=$?
    if [[ ${err} -ne 0 ]]; then
...
if [[ "${FLXGF:-}" == "YES" ]]; then
    ...
    grid_string="1p00"
    "${USHgcafs}/interp_atmos_sflux.sh" "${input_file}" "${output_file_prefix}" "${grid_string}" && true
    export err=$?
    if [[ ${err} -ne 0 ]]; then
