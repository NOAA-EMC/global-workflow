...
CYCLESH=${CYCLESH:-"${USHgcafs}/global_cycle.sh"}
REGRIDSH=${REGRIDSH:-"${USHgcafs}/regrid_gsiSfcIncr_to_tile.sh"}
export CYCLEXEC=${CYCLEXEC:-"${EXECgcafs}/global_cycle"}
...
for ((nn = 1; nn <= ntiles; nn++)); do
    cpreq "${FIXgcafs}/orog/${CASE}/${CASE}_grid.tile${nn}.nc" "${DATA}/fngrid.00${nn}"
    cpreq "${FIXgcafs}/orog/${CASE}/${CASE}.mx${OCNRES}_oro_data.tile${nn}.nc" "${DATA}/fnorog.00${nn}"
done
