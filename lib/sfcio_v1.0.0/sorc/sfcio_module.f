...
print*, "head%lsoil = ", head%lsoil
      if(ios.ne.0) return
      write(lu,iostat=ios) 4*(/8,5+29+3*head%lsoil,25,head%latb/2,head%lsoil/),&
                           4*head%irealf*(/(head%lonb*head%latb,&
                                            i=1,29+3*head%lsoil)/)
print*, "head%latb = ", head%latb
print*, "first=", 4*(/8,5+29+3*head%lsoil,25,head%latb/2,head%lsoil/)
print*, "head%lonb = ", head%lonb
print*, "head%irealf = ", head%irealf
print*, "second=", 4*head%irealf*(/(head%lonb*head%latb,&
                                            i=1,29+3*head%lsoil)/)
      if(ios.ne.0) return
      write(lu,iostat=ios) head%fhour,head%idate,head%lonb,head%latb,&
                           head%lsoil,head%irealf,(0,i=1,16)
print*, "head%fhour= ", head%fhour
print*, "head%idate = ", head%idate
print*, "head%lonb = ", head%lonb
print*, "head%latb = ", head%latb
print*, "head%lsoil = ", head%lsoil
print*, "head%irealf = ", head%irealf
...
print*, "head%lpl = ", head%lpl
print*, "head%zsoil = ", head%zsoil
      if(ios.ne.0) return
      iret=0
    elseif(head%ivs.eq.200501) then
      rewind lu
...
! write out data in netcdf format
  subroutine write_netcdf(head, data, ntiles, tile, lon, lat, orog_raw)
    type(sfcio_head),intent(in):: head
    type(sfcio_data),intent(in):: data
    real(sfcio_realkind), intent(in), optional:: lon(:,:), lat(:,:)
    real(sfcio_realkind), intent(in), optional:: orog_raw(:,:) 

    integer,         intent(in):: ntiles, tile
    ...
    integer            :: id_lon,id_lat,id_geolon,id_geolat, id_lsoil
    integer            :: id_slmsk,id_tsea,id_sheleg,id_tg3
    integer            :: id_zorl,id_alvsf,id_alvwf,id_alnsf,id_alnwf
    integer            :: id_vfrac,id_canopy,id_f10m,id_t2m,id_q2m
    integer            :: id_vtype,id_stype,id_facsf,id_facwf,id_uustar
    integer            :: id_ffmm,id_ffhh,id_hice,id_fice,id_tisfc
    integer            :: id_tprcp,id_srflag,id_snwdph,id_shdmin,id_shdmax
    integer            :: id_slope,id_snoalb,id_stc,id_smc,id_slc
    real, allocatable  :: lsoil_data(:)

    if(ntiles > 1) then
      write(outfile, '(a,i4.4,a)') 'out.sfc.tile', tile, '.nc'
    else
      outfile = "out.sfc.nc"
    endif
    ...
    error = nf_def_dim(ncid, 'lsoil', dim3, dim_lsoil)
    call netcdf_err(error, 'define dimension lsoil for file='//trim(outfile) )
    ...
    call netcdf_err(error, 'put att cartesian_axis for lsoil for file='//trim(outfile) )
...
!    error = nf_def_var(ncid, 'orog_filt', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_orog)
!    call netcdf_err(error, 'define var orog for file='//trim(outfile) )
...
!    call netcdf_err(error, 'define var orog_raw for file='//trim(outfile) )
    ...
    call netcdf_err(error, 'define var tsea for file='//trim(outfile) )
    error = nf_def_var(ncid, 'sheleg', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_sheleg)
    call netcdf_err(error, 'define var sheleg for file='//trim(outfile) )      
    error = nf_def_var(ncid, 'tg3', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_tg3)
    call netcdf_err(error, 'define var tg3 for file='//trim(outfile) )
    error = nf_def_var(ncid, 'zorl', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_zorl)
    call netcdf_err(error, 'define var zorl for file='//trim(outfile) )      
    error = nf_def_var(ncid, 'alvsf', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_alvsf)
    call netcdf_err(error, 'define var alvsf for file='//trim(outfile) )
    error = nf_def_var(ncid, 'alvwf', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_alvwf)
    call netcdf_err(error, 'define var alvwf for file='//trim(outfile) )
    error = nf_def_var(ncid, 'alnsf', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_alnsf)
    call netcdf_err(error, 'define var alnsf for file='//trim(outfile) )
    error = nf_def_var(ncid, 'alnwf', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_alnwf)
    call netcdf_err(error, 'define var alnwf for file='//trim(outfile) )
    error = nf_def_var(ncid, 'vfrac', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_vfrac)
    call netcdf_err(error, 'define var vfrac for file='//trim(outfile) )
    error = nf_def_var(ncid, 'canopy', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_canopy)
    call netcdf_err(error, 'define var canopy for file='//trim(outfile) )
    error = nf_def_var(ncid, 'f10m', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_f10m)
    call netcdf_err(error, 'define var f10m for file='//trim(outfile) )
    error = nf_def_var(ncid, 't2m', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_t2m)
    call netcdf_err(error, 'define var t2m for file='//trim(outfile) )
    error = nf_def_var(ncid, 'q2m', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_q2m)
    call netcdf_err(error, 'define var q2m for file='//trim(outfile) )
    error = nf_def_var(ncid, 'vtype', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_vtype)
    call netcdf_err(error, 'define var vtype for file='//trim(outfile) )
    error = nf_def_var(ncid, 'stype', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_stype)
    call netcdf_err(error, 'define var stype for file='//trim(outfile) )
    error = nf_def_var(ncid, 'facsf', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_facsf)
    call netcdf_err(error, 'define var facsf for file='//trim(outfile) )
    error = nf_def_var(ncid, 'facwf', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_facwf)
    call netcdf_err(error, 'define var facwf for file='//trim(outfile) )
    error = nf_def_var(ncid, 'uustar', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_uustar)
    call netcdf_err(error, 'define var uustar for file='//trim(outfile) )
    error = nf_def_var(ncid, 'ffmm', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_ffmm)
    call netcdf_err(error, 'define var ffmm for file='//trim(outfile) )
    error = nf_def_var(ncid, 'ffhh', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_ffhh)
    call netcdf_err(error, 'define var ffhh for file='//trim(outfile) )
    error = nf_def_var(ncid, 'hice', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_hice)
    call netcdf_err(error, 'define var hice for file='//trim(outfile) )
    error = nf_def_var(ncid, 'fice', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_fice)
    call netcdf_err(error, 'define var fice for file='//trim(outfile) )
    error = nf_def_var(ncid, 'tisfc', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_tisfc)
    call netcdf_err(error, 'define var tisfc for file='//trim(outfile) )
    error = nf_def_var(ncid, 'tprcp', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_tprcp)
    call netcdf_err(error, 'define var tprcp for file='//trim(outfile) )
    error = nf_def_var(ncid, 'srflag', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_srflag)
    call netcdf_err(error, 'define var srflag for file='//trim(outfile) )
    error = nf_def_var(ncid, 'snwdph', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_snwdph)
    call netcdf_err(error, 'define var snwdph for file='//trim(outfile) )
    error = nf_def_var(ncid, 'shdmin', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_shdmin)
    call netcdf_err(error, 'define var shdmin for file='//trim(outfile) )
    error = nf_def_var(ncid, 'shdmax', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_shdmax)
    call netcdf_err(error, 'define var shdmax for file='//trim(outfile) )
    error = nf_def_var(ncid, 'slope', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_slope)
    call netcdf_err(error, 'define var slope for file='//trim(outfile) )
    error = nf_def_var(ncid, 'snoalb', NF_FLOAT, 2, (/dim_lon,dim_lat/), id_snoalb)
    call netcdf_err(error, 'define var snoalb for file='//trim(outfile) )
    error = nf_def_var(ncid, 'stc', NF_FLOAT, 3, (/dim_lon,dim_lat,dim_lsoil/), id_stc)
    call netcdf_err(error, 'define var stc for file='//trim(outfile) )
    error = nf_def_var(ncid, 'smc', NF_FLOAT, 3, (/dim_lon,dim_lat,dim_lsoil/), id_smc)
    call netcdf_err(error, 'define var smc for file='//trim(outfile) )
    error = nf_def_var(ncid, 'slc', NF_FLOAT, 3, (/dim_lon,dim_lat,dim_lsoil/), id_slc)
    call netcdf_err(error, 'define var slc for file='//trim(outfile) )
      
    error = nf__enddef(ncid, header_buffer_val,4,0,4)
    call netcdf_err(error, 'end meta define for file='//trim(outfile) )
      
    !--- write out data
    error = nf_put_var_real( ncid, id_lon, lon(:dim1,1))
    call netcdf_err(error, 'write var lon for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_lat, lat(1,:dim2))
    call netcdf_err(error, 'write var lat for file='//trim(outfile) )
    allocate(lsoil_data(dim3))
    do i = 1, dim3 
       lsoil_data(i) = i
    enddo
    error = nf_put_var_real( ncid, id_lsoil, lsoil_data)
    call netcdf_err(error, 'write var lsoil for file='//trim(outfile) )
    deallocate(lsoil_data)
    error = nf_put_var_real( ncid, id_geolon, lon(:dim1,:dim2))
    call netcdf_err(error, 'write var geolon for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_geolat, lat(:dim1,:dim2))
    call netcdf_err(error, 'write var geolat for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_slmsk, data%slmsk(:dim1,:dim2))
    call netcdf_err(error, 'write var slmsk for file='//trim(outfile) )
!    error = nf_put_var_real( ncid, id_orog_raw, orog_raw(:dim1,:dim2))
!    call netcdf_err(error, 'write var orog_raw for file='//trim(outfile) )
!    error = nf_put_var_real( ncid, id_orog, data%orog(:dim1,:dim2))
!    call netcdf_err(error, 'write var orog for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_tsea, data%tsea(:dim1,:dim2))
    call netcdf_err(error, 'write var tsea for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_sheleg, data%sheleg(:dim1,:dim2))
    call netcdf_err(error, 'write var sheleg for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_tg3, data%tg3(:dim1,:dim2))
    call netcdf_err(error, 'write var tg3 for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_zorl, data%zorl(:dim1,:dim2))
    call netcdf_err(error, 'write var zorl for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_alvsf, data%alvsf(:dim1,:dim2))
    call netcdf_err(error, 'write var alvsf for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_alvwf, data%alvwf(:dim1,:dim2))
    call netcdf_err(error, 'write var alvwf for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_alnsf, data%alnsf(:dim1,:dim2))
    call netcdf_err(error, 'write var alnsf  for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_alnwf, data%alnwf(:dim1,:dim2))
    call netcdf_err(error, 'write var alnwf for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_vfrac, data%vfrac(:dim1,:dim2))
    call netcdf_err(error, 'write var vfrac for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_canopy, data%canopy(:dim1,:dim2))
    call netcdf_err(error, 'write var canopy for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_f10m, data%f10m(:dim1,:dim2))
    call netcdf_err(error, 'write var f10m for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_t2m, data%t2m(:dim1,:dim2))
    call netcdf_err(error, 'write var t2m for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_q2m, data%q2m(:dim1,:dim2))
    call netcdf_err(error, 'write var q2m for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_vtype, data%vtype(:dim1,:dim2))
    call netcdf_err(error, 'write var vtype for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_stype, data%stype(:dim1,:dim2))
    call netcdf_err(error, 'write var stype for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_facsf, data%facsf(:dim1,:dim2))
    call netcdf_err(error, 'write var facsf for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_facwf, data%facwf(:dim1,:dim2))
    call netcdf_err(error, 'write var facwf for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_uustar, data%uustar(:dim1,:dim2))
    call netcdf_err(error, 'write var uustar for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_ffmm, data%ffmm(:dim1,:dim2))
    call netcdf_err(error, 'write var ffmm for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_ffhh, data%ffhh(:dim1,:dim2))
    call netcdf_err(error, 'write var ffhh for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_hice, data%hice(:dim1,:dim2))
    call netcdf_err(error, 'write var hice for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_fice, data%fice(:dim1,:dim2))
    call netcdf_err(error, 'write var fice for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_tisfc, data%tisfc(:dim1,:dim2))
    call netcdf_err(error, 'write var tisfc for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_tprcp, data%tprcp(:dim1,:dim2))
    call netcdf_err(error, 'write var tprcp for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_srflag, data%srflag(:dim1,:dim2))
    call netcdf_err(error, 'write var srflag for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_snwdph, data%snwdph(:dim1,:dim2))
    call netcdf_err(error, 'write var snwdph for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_shdmin, data%shdmin(:dim1,:dim2))
    call netcdf_err(error, 'write var shdmin for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_shdmax, data%shdmax(:dim1,:dim2))
    call netcdf_err(error, 'write var shdmax for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_slope, data%slope(:dim1,:dim2))
    call netcdf_err(error, 'write var slope for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_snoalb, data%snoalb(:dim1,:dim2))
    call netcdf_err(error, 'write var snoalb for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_stc, data%stc(:dim1,:dim2,:dim3))
    call netcdf_err(error, 'write var stc for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_smc, data%smc(:dim1,:dim2,:dim3))
    call netcdf_err(error, 'write var smc for file='//trim(outfile) )
    error = nf_put_var_real( ncid, id_slc, data%slc(:dim1,:dim2,:dim3))
    call netcdf_err(error, 'write var slc for file='//trim(outfile) )
...
!-------------------------------------------------------------------------------
  subroutine sfcio_swdbta(lu,head,dbta,iret,lon,lat,orog_raw)
    implicit none
    integer(sfcio_intkind),intent(in):: lu
    type(sfcio_head),intent(in):: head
...
print*, "dim1, dim2, dim3 = ", dim1, dim2, dim3
    ...
    mdim3=min(&
      ...
      size(dbta%slc,3))
    iret=-5
print*, "mdim1, mdim2, mdim3=", mdim1, mdim2, mdim3
    if(mdim1.lt.dim1.or.&
       mdim2.lt.dim2.or.&
       mdim3.lt.dim3) return
    iret=-4
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    if(head%irealf.ne.2) then
      ...
      ! write out the data in netcdf format
      call write_netcdf( head, data, 1, 1, lon, lat, orog_raw)
