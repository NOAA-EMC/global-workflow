...
!
 ...
 real, intent(in)                  :: geolon_in(:,:), geolat_in(:,:)
...
!-----------------------------------------------------------------------
 print*, "at line 341, kgds_input = ", kgds_input(1)
...
!-----------------------------------------------------------------------
 print*, "kgds_output(1) = ", kgds_output(1)
...
!                              frz_h20
 ...
 real, allocatable                 :: lats_input(:,:), lons_input(:,:)
 real, allocatable                 :: lats_mismatch_output(:), lons_mismatch_output(:)
 ...
 integer :: num_mismatch_land, num_mismatch_nonland
...
!-----------------------------------------------------------------------
 ...
 print*, "count_land_output = ", count_land_output
...
! non-land
 ...
 print*, "count_nonland_output=", count_nonland_output
...
!-----------------------------------------------------------------------
 ...
 print*, "count_land_output=", count_land_output
 print*, "before line 1147, int_opt=", int_opt, ipopt(1), kgds_input(1), kgds_output_tmp(1)
 call ipolates(int_opt, ipopt, kgds_input, kgds_output_tmp,   &
...
!                       output%substrate_temp(ijsav_land_output(ij))
 ...
 num = 0     
 do ij = 1, count_land_output
   if (.not. bitmap_land_output(ij)) then   
      num = num+1
...
! For those land points that with bitmap_nonland_output=.false. use
! the nearest land points to interpolate.
 call interpolate_mismatch_points(imdl_input,jmdl_input,geolon_in*D2R,geolat_in*D2R, &
      bitmap_land_input,input%skin_temp,num_mismatch_land, &
      lons_mismatch_output*D2R,lats_mismatch_output*D2R, data_mismatch_output )
 ...
 do ij = 1, count_land_output
   if (.not. bitmap_land_output(ij)) then   
      num = num+1      
      output%skin_temp(ijsav_land_output(ij)) = data_mismatch_output(num)
   endif
 enddo     

 deallocate(lons_mismatch_output)
 deallocate(lats_mismatch_output)      
 ...
 deallocate(ibo)
 num_mismatch_nonland = 0     
 do ij = 1, count_nonland_output
   if (bitmap_nonland_output(ij)) then
     output%skin_temp(ijsav_nonland_output(ij))=output_data_nonland(ij)
   else
      num_mismatch_nonland =  num_mismatch_nonland + 1
!     output%skin_temp(ijsav_nonland_output(ij)) = 280.0 ! default value
   endif
 enddo

 allocate(lons_mismatch_output(num_mismatch_nonland))
 allocate(lats_mismatch_output(num_mismatch_nonland))      
 allocate(data_mismatch_output(num_mismatch_nonland))
 data_mismatch_output = 0
 num = 0     
 do ij = 1, count_nonland_output
   if (.not. bitmap_nonland_output(ij)) then   
      num = num+1
      lons_mismatch_output(num) = lons_nonland_output(ij)
      lats_mismatch_output(num) = lats_nonland_output(ij)
   endif
 enddo
      
 print*, "num_mismatch_nonland = ", num_mismatch_nonland     
 print*, "surface_chgres 1215", OUTPUT%SKIN_TEMP(264*768+394), &
      OUTPUT%SKIN_TEMP(264*768+395),OUTPUT%SKIN_TEMP(264*768+396)
 print*, "surface_chgres 1217", OUTPUT%SKIN_TEMP(265*768+394), &
      OUTPUT%SKIN_TEMP(265*768+395),OUTPUT%SKIN_TEMP(265*768+396)

! For those nonland points that with bitmap_nonland_output=.false. use
! the nearest nonland points to interpolate.
 call interpolate_mismatch_points(imdl_input,jmdl_input,geolon_in*D2R,geolat_in*D2R, &
      bitmap_nonland_input,input%skin_temp,num_mismatch_nonland, &
      lons_mismatch_output*D2R,lats_mismatch_output*D2R, data_mismatch_output )
 ...
 do ij = 1, count_nonland_output
   if (.not. bitmap_nonland_output(ij)) then   
      num = num+1      
      output%skin_temp(ijsav_nonland_output(ij)) = data_mismatch_output(num)
   endif
 enddo     
...
! the nearest land points to interpolate.
 call interpolate_mismatch_points(imdl_input,jmdl_input,geolon_in*D2R,geolat_in*D2R, &
      bitmap_land_input,input%snow_liq_equiv,num_mismatch_land, &
      lons_mismatch_output*D2R,lats_mismatch_output*D2R, data_mismatch_output )
 ...
 do ij = 1, count_land_output
   if (.not. bitmap_land_output(ij)) then   
      num = num+1      
      output%snow_liq_equiv(ijsav_land_output(ij)) = data_mismatch_output(num)
   endif
 enddo     
...
!$$$
 ...
 subroutine interpolate_mismatch_points(im_in, jm_in, geolon_in,geolat_in,bitmap_in,&
      data_in, num_out, lon_out,lat_out, data_out )
   integer, intent(in) :: im_in, jm_in, num_out
   real,    intent(in) :: geolon_in(im_in,jm_in), geolat_in(im_in,jm_in)
   ...
   integer, parameter :: NUMNBR2 = 60
   integer :: i_c,j_c,jstart,jend
   real    :: lon,lat
      
   print*, "im_in,jm_in = ", im_in, jm_in
   print*, "num_out = ", num_out
   print*, "max and min of lon_in is", minval(geolon_in), maxval(geolon_in)
   print*, "max and min of lat_in is", minval(geolat_in), maxval(geolat_in)   
   print*, "max and min of lon_out is", minval(lon_out), maxval(lon_out)
   print*, "max and min of lat_out is", minval(lat_out), maxval(lat_out)   
...
!      print*, "n = ", n
      ...
      i_c = 0; j_c = 0
      do j = 1, jm_in-1
         if(lat .LE. geolat_in(1,j) .and. lat .GE. geolat_in(1,j+1)) then
            j_c = j
         endif
      enddo
...
!            print*, "bitmap_in is true"
            d = spherical_distance(lon_out(n),lat_out(n),geolon_in(i,j), geolat_in(i,j))
            if( dist > d ) then
               i_in = i; j_in = j
               dist = d
            endif
      ...
      enddo; enddo
      if(i_in ==0) then  !--- search for a biggger neighbor
        print*, "use larger neighbor at n = ", n, lon, lat, num_out
        jstart = max(j_c-NUMNBR2,1)
        jend = min(j_c+NUMNBR2,jm_in)
        dist = MAX_DIST
        i_in = 0
        j_in = 0
      ...
      if(i_in ==0) then
         ...
         print*, "n=", n, lon, lat, num_out
         print*, "ERROR in interpolate_mismatch_ocean: did not find nearest points"
         call ERREXIT(4)
      endif
      data_out(n) = data_in(i_in,j_in)
   enddo   

 end subroutine interpolate_mismatch_points
