      SUBROUTINE rd_uupa (LUNUPA,LINITQ,LVLIX,cfil_hdr,LONELVL, IRET)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
C SUBPROGRAM:    RD_UUPA     READ AN UPR-AIR OBS AND GET ONE LVL DATA
C   PRGMMR: SHIMOMURA        ORG: W/NP12     DATE: 97-02-13
C
C ABSTRACT: THIS RD_UUPA SUBROUTINE IS CALLED FROM WITHIN THE PLOTPAP 
C   PROGRAM TO READ AN UPPER-AIR OBSERVATION IN ORDER TO FETCH A SINGLE
C   STATION'S OBSERVED QUANTITIES AT ONE MILLIBAR LEVEL.
C   THE INPUT UPPER-AIR OBSERVATIONAL DATA FILE IS IN AN INTERMEDIATE 
...
C   97-04-16  SHIMOMURA   MODIFIED THE BUFFERIN PARAGRAPH WHICH SEEMED
C                         TO BE NOT WAITED FOR COMPLETION
C
C USAGE: CALL RD_UUPA (LUNUPA,LINITQ,LVLIX,cfil_hdr,LONELVL, IRET)
C   INPUT ARGUMENT LIST:
C     LUNUPA - INT      UNIT NUMBER OF INPUT UPA FILE.
C
...
C                       SECTION AGAIN.
C
C     LVLIX  - INT      INDEX TO THE LEVEL DESIRED; 
C                       THIS INDEX MUST CORRESPOND TO THAT IN UPAUPA.
C
C
...
C
C   INPUT FILES:
C     UPAUPA FILE OF OBSERVATIONS WHOSE UNIT NUMBER IS LUNUPA
C
C   OUTPUT FILES:
...
C
C         . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
C         Every call to this Subr rd_uupa() will try to return one
C         station's report.  How does it save the pointers from one
C         call to the next?
...
C ...      ... THOSE HAVE BEEN SAVED IN COMMON /RDUPABFS/
      ...
      MILLIBARIX = LVLIX
      IF((MILLIBARIX .LE. 0) .OR.
     1   (MILLIBARIX .GT. 21)) THEN	!... BAD VALUE IN LVLIX
        MILLIBARIX = 5			!... USE DEFAULT LVL AT 500MB
      ENDIF
      DO  I = 1,MAXWRDLVL
        LONELVL(I) = 0
      ENDDO
...
C
  ...
  135    FORMAT(1H ,'rd_uupa: Header read is ...',/,8(8a8,/))
...
C
      IF(C8INPBUF(KNEXT).EQ.CENDFILE) THEN
          IRET = -1
          write(6,145)
  145     format(1H ,'rd_uupa: Found  NORMAL logical End-of-File')
          GO TO 999			!... EXIT
      END IF

      IF(C8INPBUF(KNEXT).NE.'STR_REPO') THEN
          write(6,165) KNEXT
  165     format(1H ,'rd_uupa: Failed to find  "STR_REPO" AT ',
     1               'C8INPBUF(',I8,')')
          PRINT *,' JOB STOPPING'
          IRET = -1
...
C ...          J = 1
C
C         ... TO MOVE ONE LEVEL'S DATA  OF ONE UPPER AIR REPORT
C
C         ... TO MOVE PREAMBLE OF FIRST 14 WORDS IN SEQUENCE ...
          KKSTART = 2*KNEXT - 1
...
C              ... REPLENISH THE I4UPABIN() ...
               LCKPT = 210
               CALL FILLUPBIN(LUNUPA,LCKPT,IRET_FIL)
               JUPABUF2 = JUPABUF
               call byteswap(JUPABUF2(1), 8, 512)
...
C             ... UPDATE THE END-RECORD POINTER ...
  ...
  225           FORMAT(1H ,'rd_uupa:BAD END-REPORT POINTER (K4ENDREP)=',
     1                     ' (',I8,')',
...
C ...                write(6,235)
C ...  235           format(1h 'rd_uupa: JOB STOPPING')
C ...                IRET = -1 
...
C ...          J = 15
          NTOTLVLS = ishft(LONELVL(13),-12)
          IB       = ishft(LONELVL(14),-12)

          IF(MILLIBARIX .LE. NTOTLVLS) THEN
            INDEXLVL = (IB - 1) + 7*(MILLIBARIX-1)
C           ... GET OBS(J) WHERE J=ARR(INDEXLVL+I),I=1,7)
            IDEST = 15 - 1
            ISRC = (KKSTART - 1) + INDEXLVL
...
C           ... add to this bit word what the old user expected ...
     ...
     1         (btest(isobserved,16))) then		!... fff
            ...
            if(btest(isobserved,17)) then		!... DZ
              IIIIOLD = ibset(iiiiold,LDZBIT)
            endif
...
C           WRITE(6,478)cnamstn(1:6), K8ENDREP,NBUFUPA
  478       FORMAT(1H ,'rd_uupa: '
     1                 'Found end-of-record ',A6,' in C8INPBUF(',I6,
     2                ') in rec no =',I6)
          KNEXT = K8ENDREP + 1
C         -----------------------------------------------------------
...
C     ---------------------------------------------------------------
  ...
  915  FORMAT(1H ,'rd_uupa:AT LCKPT=',I5,'FAILED ON PARITY ERROR WHILE ',
  ...
  925  FORMAT(1H ,'rd_uupa:AT LCKPT=',I5,'FAILED ON PHYSICAL ',
  ...
  999  CONTINUE
       ...
       SUBROUTINE FILLUPBIN(LUNUPA,LCKPT,IRET_FIL)
C                                                  21-JAN-1997/DSS
C      ...
...
C      WRITE(6,FMT='(1H ,''rd_uupa::fillupbin: was called at LCKPT='',
...
C      . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 
  910  CONTINUE
       WRITE(6,915)LCKPT,LUNUPA,NBUFUPA
  915  FORMAT(1H ,'rd_uupa::fillupbin:AT LCKPT=',I5,
  ...
  920  CONTINUE
       WRITE(6,925)LCKPT,LUNUPA,NBUFUPA
  925  FORMAT(1H ,'rd_uupa::fillupbin:AT LCKPT=',I5,
