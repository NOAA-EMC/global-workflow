...
C                                                                               
C$$$                                                                            
      SUBROUTINE REDUPAP( SRE, ERE, HDR, ARR, NRET1, IRET)
C
...
c          CALL UFBINT (LUBFR, BRR2, 4, 255, NRETZ, CATE)
...
C      write(102,187)nret2,nret1a
 187   format(' nrets ',2i10)
      DO K=1,NRETZ
          arr(1,k)=-7
         arr(2,k)=brr2(1,k)
         arr(5,k)=brr2(2,k)
...
c        is not valid!!
 ...
 89     continue
         ...
         ARR (3,K) = BRR1 (4,Km)
         ARR (4,K) = BRR1 (6,Km)
cccc         ARR (5,K) = CRR1 (2,Km)
cccc         ARR (6,K) = CRR1 (3,Km)
         ARR (7,K) = BRR1 (3,Km)
         ARR (8,K) = BRR1 (5,Km)
cccc         ARR (9,K) = CRR1 (4,Km)
c        assign winds
         if(kms .gt. 0) then
         ARR (5,K) = CRR1 (2,Kms)
         ARR (6,K) = CRR1 (3,Kms)
...
C      if(kms .gt. 0 .and. arr(1,k) .ne. crr1(1,kms))
C    1 write(102,*) ' PDIFF ', cctemp,' ',k,arr(1,k),crr1(1,kms)
C         write(102,*)NRETZ+NRET1A
