      SUBROUTINE GENTRE(Z,IMAX,JMAX,DOTSGI,A,B,M,ZLIM,ICEN1,LPLMI,
     X                  IFLO,IFHI)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
C SUBPROGRAM:  GENTRE        PLOT CENTERS FOR CGRID MAPS.
C   PRGMMR: KRISHNA KUMAR        ORG: NP12      DATE:1999-08-01
C
C ABSTRACT: PLOT CENTERS FOR CGRID MAPS.
C
C PROGRAM HISTORY LOG:
C   ??-??-??  DICK SCHURR
C   93-04-07  LILLY         CONVERT SUBROUTINE TO FORTRAN 77
C   96-02-21  LIN           CONVERT SUBROUTINE TO CFT-77
C   96-09-12  LIN           MODIFY FOR CGRID MAPS.
...
C
C USAGE:    CALL GENTRE( Z, IMAX, JMAX, DOTSGI, A, B, M, ZLIM, ICEN1,
C                        LPLMI, IFLO, IFHI )
...
C        IF(MTEXT(1:1).EQ.'0')THEN
C           LTEXT(1:1) = '0'
C           LTEXT(2:2) = CDECP(1:1)
C           LTEXT(3:4) = MTEXT(2:3)
C           MCHAR = 4
C        ELSE
C           LTEXT(1:1) = MTEXT(1:1)
C           LTEXT(2:2) = CDECP(1:1)
C           LTEXT(3:4) = MTEXT(2:3)
C           MCHAR = 4
C        ENDIF
         ...
         MCHAR = 4
         LTEXT(2:4) = MTEXT(1:3)
C        PRINT *, 'PRECIP LTEXT=', LTEXT,' MTEXT=', MTEXT
         IF (MTEXT(1:3).NE.'000') THEN
            CALL SAVLAB(ICAL,JCAL,LTEXT(1:4),TRUVAL,MCHAR)
         ENDIF
      ELSE
         LTEXT(1:1) = CDECP
         LTEXT(2:4) = MTEXT(1:3)
  ...
  900 CONTINUE
      PRINT  911, ICEN1
  911 FORMAT(1H , 'WARN -RETURN FROM CENTRE. GIVEN ARGUMENT OUT-OF-RANGE
     X. ICEN1 =HEX', Z8)
      RETURN
      END
