...
C     1  ctitleg(30:31),ctitleg(43:46)
         ...
         ifound=getenv('net',net)
c         write(cgulf(3),2713)iras(4),imont(iras(5)),iras(6),'GFS'
         if (net .eq. 'fnl' ) then
         write(cgulf(3),2713)iras(4),imont(iras(5)),iyrr,'FNL'
         else
         write(cgulf(3),2714) 'GFS',iras(4),imont(iras(5)),iyrr
         endif
 2711    format(i2,a2,x,a4,a2)
 2712    format('VT ',i2.2,'Z',1x,a3)
 2713    format(i2.2,a4,' ',i5,1x,a3) 
 2714    format(a3,1x,i2.2,a4,' ',i5) 
        if(iout1 .ne. 4) then
...
1711    format(' OBS. ',a4,' STREAMS/ISOTACH ','VALID ',i2.2,'Z ',
     1  a3,1x,i2.2,' ',a4,' ',i5)
1714    format(' OBS. ',a4,' STREAMS ','VALID ',i2.2,'Z ',
     1  a3,1x,i2.2,' ',a4,' ',i5)
 1903   format(i2,a2)
 1901   format(a4,a1)
        CALL SPCHK(IXL,0001,IAPT,IPANA,IPANY,IVARY)
c        CALL SPCHK(IXL,JXL,IAPT,IPANA,IPANY,IVARY)
       cbcd4(87:87)  = 'X'
c      CALL lPUTLAB(IXL,JXL,HT,cbcd4(1:nchar),0.0,NCHAR,1,0)
      CALL lPUTLAB(IXL,0001,HT,cbcd4(1:nchar),0.0,NCHAR,1,0)
      kprior(1)=1
      kprior(2)=1 
      ht=1.0
