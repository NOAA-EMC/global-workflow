      SUBROUTINE WNDBRK( ISTN,JSTN,KDDGD,ISPEED,NWOK,IOPTN,IRET_WNB)
C  $$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
C SUBPROGRAM:    WNDBRK      PLOTS WIND DIR AND SPEED AS SHAFT & FLAG
C   PRGMMR: SHIMOMURA        ORG: NMC412      DATE:96-10-28
C
C ABSTRACT: PLOTS WIND DIRECTION AND SPEED ON A VARIAN OR FAX MAP AS
C   ENTRIES IN THE LABEL FILE.
C   ** N O T E : TO MAKE AN EXE LOAD MODULE, WNDBRK MUST BE COMPILE
C   ** N O T E : WITH BLOCK DATA WNDCON 
C   **           (USED TO BE WINCON FOUND 'NMC.PROD.V77GRAPH.SOURCE')
C
C PROGRAM HISTORY LOG:
C   75-04-11  ORIGINAL AUTHOR DENT.
C   86-06-27  MILLER      REMOVE ASYNCHRONOUS I/O.
C   86-07-15  MILLER      MAKE SURE WIND DIRECTION IS NEVER 0.
C   89-02-21  HENRICHSEN  CLEAN UP AND DOCUMENT.
C   90-10-03  HENRICHSEN  CONVERT TO FORTRAN 77
C   96-02-20  SHIMOMURA:  CONVERT TO CRAY
C                         ADDED IRET_WNB RETURN CODE
C   96-10-24  SHIMOMURA:  ADD ERASER UNDER BARBS
C   96-10-28  SHIMOMURA:  ADJUSTING WIND-FLAG POSITIONING WITH ICORNFLAG
C
...
C
C              =  1;  WNDBRK: WARNING ... BAD IDDGD
C              =  2;  WNDBRK: WARNING ... BAD ISPEED
C              =  3;  WNDBRK: WARNING ... BAD CUTOFF VALUE
C              =  4;  WNDBRK: WARNING ... OUT-OF-RANGE FLAG COUNT
C              =  5;  WNDBRK: WARNING ... LOCATION OFF MAP
C
C   INPUT FILES:
C     FT55F001 - LABEL FILE.
C
C   OUTPUT FILES:
...
C     ...KWNDDV FORMAT ... ISTART,JSTART,KWIDTH,KHGT,MXV...
      ...
      IJLL = IOR(JLS,ILL)
      NWOK = NWOK + 1
      LABWOK(1,NWOK) = IOR(IJLL,IPRBA)		!... there goes BARBS
      LABWOK(2,NWOK) = ITEXT
...
C     ... insert the  eraser under the BARBs ...
      ...
      ITXTWRD_ERAB = IRUBB
      NWOK = NWOK + 1
      LABWOK(1,NWOK) = IJWRD_ERAB		!... ERASER UNDER BARB
cc      LABWOK(2,NWOK) = ITXTWRD_ERAB
C     . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
...
C     ...STEP (6) TO GET FLAGS FOR 100KT AND UP...
      ...
      DO  433  I=1,MORE
        ...
        IJLFL = IOR(JLS,ILLFL)
        NWOK = NWOK + 1
        LABWOK(1,NWOK) = IOR( IJLFL,IPRFL(IHEMIS) )
        LABWOK(2,NWOK) = ITEXT
...
C       ...NOW FOR THE ERASER UNDER THAT FLAG...
        ...
        IJLER = IOR(JLS,ILLER)
        NWOK = NWOK + 1
        LABWOK(1,NWOK) = IOR(IJLER,IARWUP)
        LABWOK(2,NWOK) = IAPCT
  433 CONTINUE
...
C              ... FOR CRAY VERSION, I MUST HALF-PACK  BEFORE WRITING,
               print*,'In wndbrk.f JARRAY prior to PUTL_WR'
               write(*,'(8I10)')JARRAY 
               CALL PUTL_WR(LBLTAP,NOBUF,JARRAY,LMAX2,LMAX,LPACK_RAQ,
     1                      LCLEAN_AFTQ,IRET101)
               IF(IRET101 .NE. 0) THEN
                  IF(IRET101 .EQ. -1) THEN
                    GO TO 900
                  ELSE
                    WRITE(6,FMT='(1H ,''WNDBRK::PUTL_WR::HAFPAKRA: '',
...
C              ... 
  ...
  913  FORMAT(1H ,'WNDBRK ERROR 1...BAD IDDGD = Z ',Z16, 3X,
     1       'AT I/J = Z ', Z16,1H/, Z16, 3X, 'SPEED = Z ',Z16)
       GO TO 966

  922  CONTINUE
       IRET_WNB = 2
       WRITE(6,924) ISPEED, ISTN, JSTN, IDDGD
  924  FORMAT(1H ,'WNDBRK ERROR 2...BAD ISPEED = Z ', Z16, 3X,
     1        'AT I/J = Z ', Z16, 1H/, Z16, 3X, 'IDDGD = ', Z16)
       GO TO 966
  ...
  935  FORMAT(1H ,'WNDBRK ERROR 3...BAD CUTOFF CONST FOR WND STAFF =
     1 Z ', Z16, 3X, 'AT I/J = Z ', Z16,1H/,Z16,3X, 'DIR/SPD = Z', Z16,
     2      1H/, Z16)
      GO TO 966
  ...
  946 FORMAT(1H ,'WNDBRK ERROR 4...OUT-OF-RANGE FLAG COUNT = Z',
     1       Z16, 3X, 'AT I/J = Z ', Z16,1H/,Z16,3X,'DIR/SPD = Z ', Z16,
     2       1H/, Z16)
      GO TO 966
  955 CONTINUE
...
c       write(*,*) 'test1', LABIX
  ...
  957 FORMAT(1H ,'WNDBRK ERROR 5...LOCATION OFF MAP. ISTN = Z ',
     1   Z16, 3X, 'JSTN = Z ', Z16, 3X, 'DIR/SPD = Z ', Z16, 1H/,  Z16)
      GO TO 966
  966 CONTINUE
      NWOK = 0
      GO TO 999
