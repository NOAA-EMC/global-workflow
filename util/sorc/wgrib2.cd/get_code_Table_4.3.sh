...
cat $tmp.5 | sed -e 's/ *$//' | \
  sed 's/<td>\(.*\)<td>\(.*\)/\1:\2/' >$tmp.6

cat $tmp.6 | awk -F : '{a=$1; b=$2; if (a < 192 || a > 254) \
        print "  case " a ": string=\""b"\"; break;"; \
   else print "  case " a ": if (center == 7) string=\""b"\"; else string=\"undefined\"; break;";}' >$tmp.7

cat $tmp.7 | sed -e 's/Analysis/anl/g' -e 's/Forecast/fcst/g' -e 's/Ensemble/ens/g' -e 's/Error/err/g' \
  -e 's/Observation/obs/g' -e 's/Initialization/init/g' -e 's/Bias Corrected/bias-corr/g' \
  -e 's/Probability/prob/g'  -e 's/Confidence Indicator/confidence/g'>$tmp.8
mv $tmp.8 $tmp.7

if [ $lc -eq 1 ] ; then
  cat $tmp.7 | sed -e y/ABCDEFGHIJKLMNOPQRSTUVWXYZ/abcdefghijklmnopqrstuvwxyz/ >$out
else
  cat $tmp.7 > $out
fi
cat $out
