      SUBROUTINE TEMPLP(Z,IMAX,JMAX,SCALE,A,B,M,LPLMI,IFF)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
C SUBPROGRAM:    TEMPLP      PLOT TEMPERATURE
C   PRGMMR: LUKE LIN         ORG: W/NMC412   DATE: 96-10-30
C
C ABSTRACT: PLOT TEMPERATURE.
C
C PROGRAM HISTORY LOG:
C   ??-??-??  DICK SCHURR
C   93-04-07  LILLY CONVERT SUBROUTINE TO FORTRAN 77
C   96-10-30  LUKE LIN  CONVERT IT TO CFT-77.
C
C USAGE:    CALL TEMPLP( Z, IMAX, JMAX, SCALE, A, B, M, LPLMI, IFF )
...
C     DATA ILHS/4/,IRHS/44/
C     DATA ILHSB/2/,IRHSB/46/
      DATA JLOB/9/,JHIB/57/
      DATA ILHS/13/,IRHS/53/
      DATA ILHSB/11/,IRHSB/55/
      DATA INDEF   /1.0E307 /
      DATA ITEXT/3*0/
      DATA ICIRC/'($'/
      DATA NN/2/
      ...
      DATA IKCIR/1/
      DATA JKCIR/-9/
C     ...WHERE IKCIR AND JKCIR IS DISPLACEMENT FROM PRINT TO CIRCLE
      DATA IDELTA / 0 /
      DATA JDELTA / 0 /
C
C
...
C     ...WHERE INDENT IS ALTERNATING SWITCH FOR INDENTING A ROW OR NOT.
 ...
 430  INDENT=0
      IF(J.EQ.4+7.OR.J.EQ.48+7) GO TO 431
      IF(J.EQ.8+7.OR.J.EQ.44+7) GO TO 432
      IF(J.EQ.12+7.OR.J.EQ.40+7) GO TO 433
      I = ILHSB
      I2 = IRHSB
      GO TO 440
 431  I=ILHSB+3*4
 ...
 433  I=ILHSB+4
      ...
      GO TO 440
  434 INDENT = 1
      IF(J.EQ.2+7.OR.J.EQ.50+7) GO TO 435
      IF(J.EQ.6+7.OR.J.EQ.46+7) GO TO 436
      IF(J.EQ.10+7.OR.J.EQ.42+7) GO TO 437
      I = ILHS
      I2 = IRHS
      GO TO 440
 435  I=ILHS+3*4
...
C
C     PUT CIRCLE IN LABEL ARRAY
C
      CALL PUTLAB(ILLCIR,JLLCIR,HT,ICIRC,PUTANG,NN,IPRPUT,0)
  445 CONTINUE
      I = I + 4
